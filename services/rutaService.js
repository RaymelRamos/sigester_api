...
var find = async (query) => {
    ...
    let data = await rutaModel.find(objectOfSearch)
        .select(' -__v -_id').count();
    let size = data.length
    data = await rutaModel.find(objectOfSearch)
        .select(' -__v -_id')
...
var update = async (id, body, id_company) => {
    let item = await rutaModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false },
