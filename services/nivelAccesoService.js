...
var getAll = async (query) => {
    ...
    let item = await nivelAccesoModel.find(
        {
...
var getById = async (id) => {
    return await nivelAccesoModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false }
        })
...
var find = async (query) => {
    ...
    let data = await nivelAccesoModel.find(objectOfSearch)
        .select(' -__v -_id').count();
    let size = data.length
    data = await nivelAccesoModel.find(objectOfSearch)
        .select(' -__v -_id')
...
var create = async (body) => {
    let item = new nivelAccesoModel(body);
...
var update = async (id, body, id_company) => {
    let item = await nivelAccesoModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false },
    ...
    if (item == null) {
        ...
        await item.save();
        item = await nivelAccesoModel.findOne(
            { 
                uuid: id, 
                deletedAt: { $exists: false },
                id_company: id_company
