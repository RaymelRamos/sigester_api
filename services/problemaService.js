...
var getById = async (id) => {
    return await problemaModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false }
        })
...
var find = async (query) => {
    ...
    let data = await problemaModel.find(objectOfSearch)
        .select(' -__v -_id').count();
    let size = data.length
    data = await problemaModel.find(objectOfSearch)
        .select(' -__v -_id')
...
var create = async (body) => {
    let item = new problemaModel(body);
...
var update = async (id, body, id_company) => {
    let item = await problemaModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false },
    ...
    if (item == null) {
        ...
        await item.save();
        item = await problemaModel.findOne(
            { 
                uuid: id, 
                deletedAt: { $exists: false },
                id_company: id_company
