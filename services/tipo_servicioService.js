...
var getById = async (id) => {
    return await tipo_servicioModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false }
        })
...
var find = async (query) => {
    ...
    let data = await tipo_servicioModel.find(objectOfSearch)
        .select(' -__v -_id').count();
    let size = data.length
    data = await tipo_servicioModel.find(objectOfSearch)
        .select(' -__v -_id')
...
var create = async (body) => {
    let item = new tipo_servicioModel(body);
...
var update = async (id, body, id_company) => {
    let item = await tipo_servicioModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false },
    ...
    if (item == null) {
        ...
        await item.save();
        item = await tipo_servicioModel.findOne(
            { 
                uuid: id, 
                deletedAt: { $exists: false },
                id_company: id_company
