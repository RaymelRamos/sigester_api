...
var getAll = async (query) => {
    ...
    let item = await parametrizationModel.find(
        {
...
var getById = async (id) => {
    return await parametrizationModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false }
        })
...
var find = async (query) => {
    ...
    let data = await parametrizationModel.find(objectOfSearch)
        .select(' -__v -_id').count();
    let size = data.length
    data = await parametrizationModel.find(objectOfSearch)
        .select(' -__v -_id')
...
var create = async (body) => {
    let item = new parametrizationModel(body);
...
var update = async (id, body, id_company) => {
    let item = await parametrizationModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false },
    ...
    if (item == null) {
        ...
        await item.save();
        item = await parametrizationModel.findOne(
            { 
                uuid: id, 
                deletedAt: { $exists: false },
                id_company: id_company
