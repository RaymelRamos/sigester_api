...
var getAll = async (query) => {
    ...
    let item = await usuarioModel.find(
        {
            deletedAt: { $exists: false }
        })
        .select(' -__v -_id').skip(skip).limit(limit).lean();
    let size = await usuarioModel.count(
        {
            deletedAt: { $exists: false }
        });
...
var find = async (query) => {
    ...
    let data = await usuarioModel.find(objectOfSearch)
        .select(' -__v -_id').count();
    let size = data.length
    data = await usuarioModel.find(objectOfSearch)
        .select(' -__v -_id')
...
var create = async (body) => {
    ...
    let item = new usuarioModel(
        {
            nombre_usuario: body.nombre_usuario ? body.nombre_usuario : item.nombre_usuario,
            email: body.email ? body.email : item.email,
            div_territorial_id: body.div_territorial_id ? body.div_territorial_id : item.div_territorial_id,
            nivel_acceso_id: body.nivel_acceso_id ? body.nivel_acceso_id : item.nivel_acceso_id,
            module: body.module ? body.module : item.module,
            role: body.role ? body.role : item.role,
        });
...
var update = async (id, body, id_company) => {
    let item = await usuarioModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false }
        })
    if (item == null) {
        ...
        await item.save();
        item = await usuarioModel.findOne(
            { 
                uuid: id, 
                deletedAt: { $exists: false }
            })
