...
var getById = async (id) => {
    return await procedenciaModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false }
        })
...
var find = async (query) => {
    ...
    let data = await procedenciaModel.find(objectOfSearch)
        .select(' -__v -_id').count();
    let size = data.length
    data = await procedenciaModel.find(objectOfSearch)
        .select(' -__v -_id')
...
var create = async (body) => {
    let item = new procedenciaModel(body);
...
var update = async (id, body, id_company) => {
    let item = await procedenciaModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false },
    ...
    if (item == null) {
        ...
        await item.save();
        item = await procedenciaModel.findOne(
            { 
                uuid: id, 
                deletedAt: { $exists: false },
                id_company: id_company
