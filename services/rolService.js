...
var find = async (query) => {
    ...
    let data = await rolModel.find(objectOfSearch)
        .select(' -__v -_id').count();
    let size = data.length
    data = await rolModel.find(objectOfSearch)
        .select(' -__v -_id')
...
var update = async (id, body, id_company) => {
    let item = await rolModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false },
