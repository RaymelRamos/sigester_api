...
var getAll = async (query) => {
    ...
    let item = await grupoModel.find(
        {
            deletedAt: { $exists: false }
        })
        .select(' -__v -_id').skip(skip).limit(limit).lean();
    let size = await grupoModel.count(
        {
            deletedAt: { $exists: false }
        });
...
var getById = async (id) => {
    return await grupoModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false }
        })
...
var find = async (query) => {
    ...
    let data = await grupoModel.find(objectOfSearch)
        .select(' -__v -_id').count();
    let size = data.length
    data = await grupoModel.find(objectOfSearch)
        .select(' -__v -_id')
...
var update = async (id, body, id_company) => {
    let item = await grupoModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false },
