...
var getById = async (id) => {
    return await brasModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false }
        })
...
var find = async (query) => {
    ...
    let data = await brasModel.find(objectOfSearch)
        .select(' -__v -_id').count();
    let size = data.length
    data = await brasModel.find(objectOfSearch)
        .select(' -__v -_id')
...
var update = async (id, body, id_company) => {
    let item = await brasModel.findOne(
        { 
            uuid: id, 
            deletedAt: { $exists: false },
